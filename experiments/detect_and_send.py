...
parser.add_argument('--send_zmq', action='store_true', help='enable this if sending data through zmq', default=False)
...
def detect(opt):
    if opt.send_zmq:
        ...
        producer.bind("tcp://0.0.0.0:5556")
    ...
    #        print('Batch %d... (Done %.3fs)' % (batch_i, time.time() - prev_time))
            t_end = time.time()
            t_inf = round((t_end - t_start)*1000,3)
            if opt.send_zmq:
                producer.send_string(str(t_inf))
